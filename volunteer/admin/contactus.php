...
echo "<div class='container'>";
  ...
  if(isset($_GET['do']) and $_GET['do']=='rv'){
       ...
       $count_supp=$cto_db->select("*","contactus");//get all row and count it
     ...
     $fetch_limt_data=$cto_db->select_p("*","contactus ORDER BY `contactus`.`id` DESC",$start_page,20);


 


       echo "<br> <h3 align='center'> الرسائل </h3>";
          ...
          echo "<tbody>";
                 ...
                 foreach ($fetch_limt_data as $data) {
                  ...
                  echo "<tr>";
                    ...
                    echo "<td>".'<textarea rows="2" cols="15">'.$data['note']."</textarea></td>";
...
<?php
    ...
    elseif(isset($_GET['do']) and $_GET['do']==md5('delete')  and isset($_GET['id']) and is_numeric($_GET['id']) ){
      ...
      $check=$cto_db->select("id","contactus","id=$id");
      if ($check>0) {
        
        $del=$cto_db->delete_r("contactus","id",$id);
        if($del=="yes"){
